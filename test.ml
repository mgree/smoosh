...
(*val check_expansion :
  ...
  result*)
let check_expansion (test_name, s0, w_in, w_expected):result= 
  (let (s1, w_out) = (fully_expand_words s0 Unquoted w_in) in
  if (listEqualBy (=) w_out w_expected)
  then Ok
...
let expansion_tests:(string*ty_os_state*(entry)list*(entry)list)list=  
 ([
    ("plain string foo", os_empty, [S "foo"], [S "foo"]);
    ("expand tilde without username", { os_empty with shell_env = (Pmap.add "HOME" "/home/testuser" os_empty.shell_env) }, [K Tilde], [S "/home/testuser"]);
