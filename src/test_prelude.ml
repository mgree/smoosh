...
let checker (test_fn : 'a -> 'b) (equal : 'b -> 'c -> bool)
  ...
  if equal out expected_out then Ok
...
let test_part name checker stringOfExpected tests count failed =
  List.iter
    (fun t ->
      ...
      | Ok -> incr count
      | Err e ->
          let expected_str = stringOfExpected e.expected in
          let got_str = stringOfExpected e.got in
          let results_are_multiline =
            String.contains expected_str '\n' || String.contains got_str '\n'
          in

          if results_are_multiline then
            Printf.printf
              "%s test '%s' failed: \nexpected \n'%s' \ngot \n'%s' \n\n" name
              e.msg expected_str got_str
          else
...
let add_literal_env_string k v s0 =
  internal_set_param k (symbolic_string_of_string v) s0
...
let os_var_x_set_three : symbolic os_state =
  add_literal_env_string "x" "\"this is three\"" os_empty

let os_var_x_five : symbolic os_state = add_literal_env_string "x" "5" os_empty

let os_ifs_spaceandcomma : symbolic os_state =
  add_literal_env_string "IFS" " ," os_empty
...
let fresh (fs : fs_mut) : fs_mut =
  { parent = fs.parent; contents = Pmap.map (fun x -> x) fs.contents }
...
let set_fs (fs : fs_mut) (st : symbolic os_state) : symbolic os_state =
  ...
  {
    st with
    symbolic = { st.symbolic with fs_root = root };
    sh = { st.sh with cwd = "/" };
  }
...
let os_complicated_fs_in_a =
  { os_complicated_fs with sh = { os_complicated_fs.sh with cwd = "/a" } }
